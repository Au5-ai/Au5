...
- **Swagger UI**: http://localhost:1366/swagger
- **API Explorer**: http://localhost:1366/swagger/v1/swagger.json
...
- `POST /api/transcriptions` - Submit transcription data
...
### Managing the Backend Container

**Docker:**
```bash
# View container status
docker ps --filter "name=au5-backend"

# View logs
docker logs au5-backend -f

# Stop the container
docker stop au5-backend

# Restart the container
docker restart au5-backend

# Remove the container
docker rm au5-backend
```

**Podman:**
```bash
# View container status
podman ps --filter "name=au5-backend"

# View logs
podman logs au5-backend -f

# Stop the container
podman stop au5-backend

# Restart the container
podman restart au5-backend

# Remove the container
podman rm au5-backend
```
...
# Search for errors
docker logs au5-backend 2>&1 | grep -i error

# Export logs to file
docker logs au5-backend > backend-logs.txt 2>&1
```
...
- **Database**: Entity Framework with query optimization
