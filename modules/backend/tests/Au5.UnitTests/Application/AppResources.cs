namespace Au5.UnitTests.Application;

public class AppResources
{
	public const string FailedToConfigSystem = "Failed To Config System";
	public const string InvalidDirection = "Direction must be either 'ltr' or 'rtl'.";
	public const string InvalidLanguageFormat = "InvalidLanguageFormat";
	public const string InvalidUrl = "Invalid Url";
	public const string InvalidUsernameOrPassword = "Username or password is incorrect.";
	public const string SystemAlreadyConfigured = "System Already Configured , You can set ForceUpdate=true.";
	public const string SystemIsNotConfigured = "SystemIsNotConfigured";
	public const string Required = "property can not be empty";
	public const string UnAuthorizedAction = "Unauthorized user, You can't do this action.";

	public const string InvalidUsernameFormat = "Invalid Username format.";
	public const string InvalidPasswordFormat = "Password must be at least 6 characters long.";
}
