...
// export function injectLocalScript(fileName: string, callback: () => void = () => {}): void {
 ...
 */
export async function waitForMatch(selector: string, text?: string): Promise<HTMLElement> {
  const matchesText = (el: Element): boolean => (text ? el.textContent?.trim() === text : true);
 ...
 * @returns The matching HTMLElement or null
 */
export function selectSingle(selector: string): HTMLElement | null {
  return document.querySelector<HTMLElement>(selector);
}
 ...
 * @returns Array of matching HTMLElements
 */
export function selectAll(selector: string, textPattern?: string): HTMLElement[] {
  const elements = Array.from(document.querySelectorAll<HTMLElement>(selector));
 ...
 * @param opacity - Opacity value (e.g., "0.5")
 */
export function setOpacity(container: HTMLElement, applyToSelf: boolean, opacity: string): void {
  if (applyToSelf) {
    container.style.opacity = opacity;
  } else {
...
export function getDomContainer(
  ariaSelector: string,
  fallbackSelector: string
): {container: HTMLElement | null; usedAria: boolean} {
 ...
 */
export function injectScript(fileName: string, onLoad: () => void = () => {}): void {
  const script = document.createElement("script");
  script.src = chrome.runtime.getURL(fileName);
  script.type = "text/javascript";
