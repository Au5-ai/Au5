...
class MeetingHubClient {
  constructor(meetingId) {
    ...
    this.connection = new HubConnectionBuilder().withUrl(HubConnectionConfig.hubUrl).withAutomaticReconnect().withHubProtocol(new MessagePackHubProtocol()).build();
    this.setupHandlers();
  ...
  setupWindowMessageListener() {
    window.addEventListener("message", (event) => {
      ...
      switch (action) {
        case "StartTranscription":
          this.connection.invoke(action, { MeetingId: this.meetingId, UserId: payload.userId });
          break;
        case "RealTimeTranscription":
